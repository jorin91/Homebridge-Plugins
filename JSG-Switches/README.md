...
- Generates plugin-prefixed accessory IDs when a device has no saved ID.
...
When no usable `id` is present, the plugin generates `jsg-switches-{normalized-device-name}` as a fallback and writes it into Homebridge `config.json`. If multiple devices generate the same ID, later generated IDs receive deterministic suffixes such as `-1` and `-2` based on config order. Explicit configured IDs must be unique.

During migration from an older version without a saved ID, the plugin also checks the previous name-based accessory UUID. An existing cached accessory is linked to the new generated ID instead of being replaced, which keeps existing Homebridge and automation references intact. Plain and interval switch state stored under the previous ID is also preserved under the new ID.
...
- Device `id` is optional. A stored ID is authoritative after normalization and is only rewritten when normalization changes it.
- Missing IDs use `jsg-switches-{normalized-device-name}` and are written back to Homebridge `config.json`.
...
[JSG-Switches-v0.1.6](https://github.com/jorin91/Homebridge-Plugins/releases/tag/JSG-Switches-v0.1.6)

```powershell
npm install -g "https://github.com/jorin91/Homebridge-Plugins/releases/download/JSG-Switches-v0.1.6/JSG-Switches-0.1.6.tgz"
```
