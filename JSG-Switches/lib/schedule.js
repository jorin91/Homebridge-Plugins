...
function normalizeSchedule(rawEntries, log, ownerLabel) {
  ...
  if (!Array.isArray(rawEntries)) {
    writeWarning(log, `${formatOwner(ownerLabel)} schedule entries must be an array. The scheduled switch will use its default state until a valid range is configured.`);
    return [];
  }
...
function formatOwner(ownerLabel) {
  return ownerLabel ? `JSG-Switches device '${ownerLabel}'` : 'JSG-Switches device';
}
